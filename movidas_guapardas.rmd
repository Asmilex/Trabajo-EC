...
Usaremos el dataframe `Seatbelts` para el análisis, y por simplicidad solo trabajaremos con los datos completos del mismo:

<!-- NOTE - SeatBelts no tiene NAs -->
...
```{r}
fit <- lm(DriversKilled ~ drivers, data = tsSB)
```
...
```{r, w = 7, h = 4}
fit <- lm(DriversKilled ~ drivers + kms, data = tsSB)
visreg(fit, xvar = "drivers")
```

En particular, observamos que:

```{r}
fit <- lm(DriversKilled ~ kms + VanKilled, data = tsSB)
visreg(fit, xvar = "VanKilled")
```

Pero ajustando por número de conductores:
```{r}
fit <- lm(DriversKilled ~ drivers + kms + VanKilled, data = tsSB)
visreg(fit, xvar = "VanKilled")
```

Y además podemos dividirlo según los valores de un factor:

```{r, w = 7, h = 4}
fit <- lm(DriversKilled ~ drivers + kms + law + VanKilled, data = tsSB)
visreg(fit, xvar = "drivers", by = "law")
```

En realidad, puede ser cualquier variable. Si es continua, se puede indicar cómo se dividen por el argumento `breaks`.
```{r}
fit <- lm(DriversKilled ~ drivers + kms + law, data = tsSB)
visreg(fit, xvar = "drivers", by = "kms", breaks = 4)
```

Otra opción interesante es superponer los gráficos anteriores en una sola gráfica:

```{r, w = 7, h = 7}
visreg(fit, xvar = "drivers", by = "law", overlay = TRUE)
```

También se pueden hacer contrastes:
```{r}
visreg(fit, xvar = "law", type = "contrast")
```

En modelos más complicados se pueden hacer gráficas de contour:

```{r, w = 7, h = 7}
fit <- lm(DriversKilled ~ poly(kms, 2) * poly(drivers, 2) + law, data = tsSB)
visreg2d(fit, "kms", "drivers")
visreg2d(fit, "kms", "drivers", plot.type = "gg")
visreg2d(fit, "kms", "drivers", plot.type = "persp")
# TODO: persp queda un poco extraño
```
