...
En este trabajo estudiaremos de forma visual el conjunto de datos `Seatbelts`, el cual recoge la información los accidentes de tráfico mortales en Gran Bretaña comprendidos entre los años 1969 y 1984. La ley relacionada con la obligatoriedad de los cinturones se introdujo el 31 de enero de 1983.
...
También puede mostrarse con cuadrados, lo cual se asemeja a un *heatmap*:
...
Como la matriz de correlaciones es simétrica, podemos mostrar únicamente una o la triangular superior o la inferior.
...
Podemos modificar los colores que se usan en las gráficas que vienen por defecto. Esto se puede hacer con el argumento `col`, al que le pasaremos los colores que queremos usar. Por ejemplo, podemos usar colores secuenciales, tales como `YlGn`, `OrRd`, `YlOrRd`, `YlOrBr`, `Blues`, `Reds`, `Greens`, `Purples`, etc; o colores divergentes, tales como `RdBu`, `BrBG`, `PiYG`, `PRGn`, `PuOr` o `RdYlBu`. Los colores generados pueden ser obtenidos mediante las funciones `COL1` o `COL2`. El primero se encarga de interpolar entre los dos extremos, mientras que el segundo genera paletas divergentes.
...
La función `corrplot()` puede visualizar los p-valores y el intervalo de confianza de la matriz de correlaciones. Por ejemplo, para un nivel de confianza $\alpha = 0.95$:
...
Una opción interesante es dejar en blanco las entradas para las que falla el test, y añadimos los coeficientes para los que sí:
...
```{r, w = 7, h = 7}
corrplot(
    corSB, lowCI = test$lowCI, uppCI = test$uppCI, order = "hclust",
...
En el gráfico anterior observamos la relación lineada estimada por el modelo y una banda de confianza sobre esa estimación.
...
Esto se debe a que los datos corresponden a una serie temporal, y por tanto el análisis no corresponde a una relación mes a mes.
