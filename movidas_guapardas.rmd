...
```{r}
fit <- lm(DriversKilled ~ kms + VanKilled, data = SB)
...
```{r}
fit <- lm(DriversKilled ~ drivers + kms + VanKilled, data = SB)
visreg(fit, xvar = "VanKilled")
summary(fit)
```

Es decir, VanKilled puede ser un factor de confusión.

Y además podemos dividirlo según los valores de un factor:

```{r, w = 7, h = 4}
fit <- lm(DriversKilled ~ drivers + kms + law + VanKilled, data = SB)
...
```{r, w = 7, h = 7}
fit <- lm(DriversKilled ~ poly(kms, 2) * poly(drivers, 2) + law, data = SB)
