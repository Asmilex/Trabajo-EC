...
```{r}
SB <- data.frame(na.omit(Seatbelts))
summary(SB)
```
...
Muestra las correlaciones entre cada una de las variables de dos formas simultáneamente:
...
Otras opciones para el valor de `method` son `square`, `ellipse`, `shade` o `pie`.

## Cambiando el orden de las variables

También podemos cambiar el orden de las variables en la representación como se muestra:
...
Con la función `visreg()` podemos visualizar el modelo anterior:
...
También podemos hacer visualizaciones con varias variables explicativas, como por ejemplo podemos visualizar el modelo que describe el número de conductores fallecidos en función del número de conductores en un cierto año y la distancia recorrida:

```{r, w = 7, h = 7}
fit <- lm(DriversKilled ~ drivers + kms, data = SB)
visreg(fit, xvar = "kms")
```
...
```{r}
fit <- lm(DriversKilled ~ drivers + kms + law, data = SB)
visreg(fit, xvar = "kms", by = "law")
```
